...
public class BinanceTradeMarginServiceRaw extends BinanceBaseService {
  ...
  public MarginPair getMarginPair(CurrencyPair symbol) throws IOException {
    Long recvWindow =
            (Long) exchange.getExchangeSpecification().getExchangeSpecificParametersItem("recvWindow");
    return binance.getMarginPair(
            BinanceAdapters.toSymbol(symbol), recvWindow, getTimestamp(), apiKey, signatureCreator);
  }

  public List<MarginAsset> getAllMarginAssets() throws IOException {
    Long recvWindow =
            (Long) exchange.getExchangeSpecification().getExchangeSpecificParametersItem("recvWindow");
    return binance.getAllMarginAssets(recvWindow, getTimestamp(), apiKey, signatureCreator);
  }

  public List<MarginPair> getAllMarginPairs() throws IOException {
    Long recvWindow =
            (Long) exchange.getExchangeSpecification().getExchangeSpecificParametersItem("recvWindow");
    return binance.getAllMarginPairs(recvWindow, getTimestamp(), apiKey, signatureCreator);
  }

  public MarginPriceIndex getMarginPriceIndex(CurrencyPair pair) throws IOException {
    Long recvWindow =
            (Long) exchange.getExchangeSpecification().getExchangeSpecificParametersItem("recvWindow");
    return binance.getMarginPriceIndex(
            BinanceAdapters.toSymbol(pair), recvWindow, getTimestamp(), apiKey, signatureCreator);
  }
}
